...
nobel <- read_csv("https://raw.githubusercontent.com/mine-cetinkaya-rundel/feed-me-back/master/examples/01-learnr-gradethis/data/nobel.csv")
...
to load this package. Remember that it will report back to you with messaging 
about the versions of tidyverse packages it loads and any conflicts with 
function names in base packages.
...
The dataset for this assignment can be found as a csv file at https://bit.ly/nobel-laureates. You can read it in directly using the `read_csv()` function.
...
How many nobel laureates fit the criteria? Use the code chunk below to obtain this number.
...
grade_result(
  pass_if(~ identical(.result, 228L), "Great job! There are 228 nobel laureates that fit the criteria."),
